...
@export var particule_scene: PackedScene
...
func _ready() -> void:
	_particule_scene = particule_scene
	_create_particules()
	create_noise()
...
func compute_velocity(p: Particule) -> void:
	var boid_force = get_boids_force(p, _particules).normalized() 
...
func change_queen(new_queen: Node2D) -> void:
	queen = new_queen
